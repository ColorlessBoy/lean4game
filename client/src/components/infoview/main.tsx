...
} from "../../../../node_modules/lean4-infoview/src/infoview/util";
import {
  EditorContext,
  ConfigContext,
  ProgressContext,
  VersionContext,
} from "../../../../node_modules/lean4-infoview/src/infoview/contexts";
import {
  RpcContext,
  WithRpcSessions,
  useRpcSessionAtPos,
} from "../../../../node_modules/lean4-infoview/src/infoview/rpcSessions";
...
import {
  Typewriter,
  getInteractiveDiagsAt,
  hasErrors,
  hasInteractiveErrors,
} from "./typewriter";
...
function DualEditorMain({
  ...
  React.useEffect(() => {
    if (proof?.completed) {
      dispatch(
        levelCompleted({ game: gameId, world: worldId, level: levelId })
      );
      ...
      let newInv = [...inv, ...newTiles].filter(
  ...
  const config =
    useEventResult(ec.events.changedInfoviewConfig) ?? defaultInfoviewConfig;
...
function ExerciseStatement({ data, showLeanStatement = false }) {
  ...
  if (!(data?.descrText || data?.descrFormat)) {
    return <></>;
  }
  return (
    <>
      <div className="exercise-statement" data-content={t('Exercise')}>
        {data?.descrText && (
          <Markdown>
            {(data?.displayName
              ? `**Theorem** \`${data?.displayName}\`: `
              : "") + t(data?.descrText, { ns: gameId })}
          </Markdown>
        )}
        {data?.descrFormat && showLeanStatement && (
          <p>
            <code className="lean-code">{data?.descrFormat}</code>
          </p>
        )}
      </div>
    </>
...
export function Main(props: { world: string; level: number; data: LevelInfo }) {
  ...
  const { setDeletedChat, showHelp, setShowHelp } =
    React.useContext(DeletedChatContext);
  ...
  const curUri = useEventResult(
    ec.events.changedCursorLocation,
    (loc) => loc?.uri
  );

  const curPos: DocumentPosition | undefined = useEventResult(
    ec.events.changedCursorLocation,
      ...
      <div className="infoview vscode-light">
        {proof?.completedWithWarnings && (
          <div className="level-completed">
            {proof?.completed
              ? t("Level completed! 🎉")
              : t("Level completed with warnings 🎭")}
          </div>
        )}
        <Infos />
        <Hints
          hints={proof?.steps[curPos?.line]?.goals[0]?.hints}
          showHidden={showHelp.has(curPos?.line)}
          step={curPos?.line}
          selected={selectedStep}
          toggleSelection={toggleSelection(curPos?.line)}
  ...
  if (isLastStepWithErrors(proof, i)) {
    ...
    return (
      <div className="command">
        ...
        <Button
          to=""
          className="undo-button btn btn-inverted"
          title={t("Retry proof from here")}
          onClick={deleteProof}
        >
...
function GoalsTabs({
  ...
  proofStep: InteractiveGoalsWithHints;
  last: boolean;
  onClick?: any;
  ...
  return (
    <div className="goal-tabs" onClick={onClick}>
      <div className={`tab-bar ${last ? "current" : ""}`}>
        {proofStep.goals.map((goal, i) => (
          // TODO: Should not use index as key.
          <div
            key={`proof-goal-${i}`}
            className={`tab ${i == selectedGoal ? "active" : ""}`}
            onClick={(ev) => {
              onGoalChange(i);
              setSelectedGoal(i);
              ev.stopPropagation();
      ...
      <div className="goal-tab vscode-light">
        <Goal
          typewriter={false}
          filter={goalFilter}
          goal={proofStep.goals[selectedGoal]?.goal}
        />
      </div>
    </div>
  );
...
export function TypewriterInterfaceWrapper(props: {
  world: string;
  level: number;
  data: LevelInfo;
  worldSize: number;
...
export function TypewriterInterface({ props }) {
  ...
  const { proof, setProof, crashed, setCrashed, interimDiags } =
    React.useContext(ProofContext);
      ...
      proof?.steps.slice(line).map((step, i) => {
        ...
        deletedChat = [
          ...
          ...filteredHints.filter(
            (hint) => !hint.hidden || showHelp.has(line + i)
          ),
        ];
      });
  ...
  React.useEffect(() => {
    if (typeof selectedStep !== "undefined") {
      Array.from(
        proofPanelRef.current?.getElementsByClassName(`step-${selectedStep}`)
      ).map((elem) => {
  ...
  let lastStepErrors = proof?.steps.length
    ? hasInteractiveErrors(getInteractiveDiagsAt(proof, proof?.steps.length))
    : false;
  ...
  return (
    <div className="typewriter-interface">
      <RpcContext.Provider value={rpcSess}>
        <div className="content">
          ...
          <div className="proof" ref={proofPanelRef}>
            {mobile && props.data?.introduction && (
              <div
                className={`message information step-0${
                  selectedStep === 0 ? " selected" : ""
                }`}
                onClick={toggleSelectStep(0)}
              >
              ...
              crashed ? (
                  ...
                  {interimDiags.map((diag) => {
                    ...
                    return (
                      <div>
                        <div className={`${severityClass} ml1 message`}>
                          <p className="mv2">
                            {t("Line")}&nbsp;{diag.range.start.line},{" "}
                            {t("Character")}&nbsp;{diag.range.start.character}
                          </p>
                          <pre className="font-code pre-wrap">
                            {diag.message}
                          </pre>
                ...
                <>
                        ...
                        >
                          ...
                          {mobile && (
                            <Hints
                              ...
                              step={i}
                              selected={selectedStep}
                              toggleSelection={toggleSelectStep(i)}
                            />
                          )}
                          ...
                          {!isLastStepWithErrors(proof, i) && (
                            <GoalsTabs
                              ...
                              last={
                                i ==
                                proof?.steps.length -
                                  (lastStepHasErrors(proof) ? 2 : 1)
                              }
                              onClick={toggleSelectStep(i)}
                              onGoalChange={
                                i ==
                                proof?.steps.length -
                                  (lastStepHasErrors(proof) ? 2 : 1)
                                  ? (n) => setDisableInput(n > 0)
                                  : (n) => {}
                              }
                            />
                          )}
                  ...
                  {proof?.diagnostics.length > 0 && (
                    <div
                      key={`proof-step-remaining`}
                      className="step step-remaining"
                    >
                      <Errors
                        errors={proof?.diagnostics}
                        typewriterMode={true}
                      />
                    </div>
                  )}
                  {mobile && proof?.completed && (
                    <div className="button-row mobile">
                      {props.level >= props.worldSize ? (
                        ...
                        <Button
                          to={`/${gameId}/world/${props.world}/level/${
                            props.level + 1
                          }`}
                        >
                          {t("Next")}&nbsp;
                ...
                <CircularProgress
                  variant="determinate"
                  value={100 * (1 - 1.024 ** -loadingProgress)}
                />
              )
